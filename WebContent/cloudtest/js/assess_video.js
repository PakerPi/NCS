...
function assess_select() {
	...
	// ajax传输
	$.ajax({
		url: 'assessManageAction_findAssessVideo',
	    type: "POST",  
	    async: false,  
	    cache: false, 
	        ...
	        if(data!=null){
        		...
        		for(var i in jsondata){        			
        			var str = "{\"assessId\":\"" + jsondata[i][0].assessId + "\","
				    		+ "\"aimId\":\"" + jsondata[i][2].videoId + "\","
				    		+ "\"aimName\":\"" + jsondata[i][2].videoName + "\","
...
function refresh(){
	...
	// ajax传输
	$.ajax({
		url: 'assessManageAction_showAllAssessVideo',
	    type: "POST",  
	    async: false,  
	    cache: false, 
	    processData: false,// 告诉jQuery不要去处理发送的数据
...
function getTable(mydata) {
	...
	$("#table_list_2").jqGrid( {
		...
		rowNum:20,
		rowList:[10,20,30],
		colNames:["编号","评价人","微课ID","微课名称","评分","评价时间","评价内容"],
		colModel:[
