...
function assess_select() {
	...
	// ajax传输
	$.ajax({
		url: 'assessManageAction_findAssessCourse',
	    type: "POST",  
	    async: false,  
	    cache: false, 
	        ...
	        if(data!=null){
        		...
        		for(var i in jsondata){        			
        			var str = "{\"assessId\":\"" + jsondata[i][0].assessId + "\","
				    		+ "\"aimId\":\"" + jsondata[i][2].courseId + "\","
				    		+ "\"aimName\":\"" + jsondata[i][2].courseName + "\","
...
function refresh(){
	...
	// ajax传输
	$.ajax({
		url: 'assessManageAction_showAllAssessCourse',
	    type: "POST",  
	    async: false,  
	    cache: false, 
	    processData: false,// 告诉jQuery不要去处理发送的数据
...
function getTable(mydata) {
	...
	$("#table_list_2").jqGrid( {
		...
		rowNum:20,
		rowList:[10,20,30],
		colNames:["编号","评价人","课程ID","课程名称","评分","评价时间","评价内容"],
		colModel:[
